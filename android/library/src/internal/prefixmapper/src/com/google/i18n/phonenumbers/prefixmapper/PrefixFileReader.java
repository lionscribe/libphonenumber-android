...
public class PrefixFileReader {
  ...
  private InputStream openAssetStream(String path) throws IOException
  {
    // We replace the leading slash if it exists
    return context.getAssets().open(path.replaceAll("^/+", ""));
  }
  ...
  private void loadMappingFileProvider() {
    ...
    try {
      InputStream source = openAssetStream(phonePrefixDataDirectory + "config");
  ...
  private void loadPhonePrefixMapFromFile(String fileName) {
    ObjectInputStream in = null;
    try {
      InputStream source = openAssetStream(phonePrefixDataDirectory + fileName);
      in = new ObjectInputStream(source);
